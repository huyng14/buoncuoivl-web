...
//            $db= new PDO("mysql:host= localhost;dbname=buoncuoi","root","");
        ...
        $query= "SELECT * FROM user WHERE ten_user= '$userName' AND password='$pass' AND trang_thai=1";
        $rows= $db->query($query);
        if($rows != NULL){
            $r= $rows->fetch();
        }
        ...
        if($r != null){
            ...
            setcookie('admin_buoncuoi', 'logged', time()+3600);
            setcookie('ma_user',$r[0],  time()+3600);
            setcookie('tenUser', "$userName", time()+3600);
...
?>
                                <div class="checkbox checkbox-circle checkbox-info">
                                        <input name="chbSign" id="checkbox7" type="checkbox" checked>
                                        <label for="checkbox7">
                                                Keep me signed in
                                        </label>
                                </div>
